...
for (const imgUrl of imgList) {
  ...
  const imgFileName = imgUrl
    .replace(
      'https://media-cdn-zspms.kurogame.com/pnswebsite/website2.0/images/',
      ''
    )
    .replace(
      'https://media-cdn-zspms.kurogame.net/pnswebsite/website2.0/images/',
      ''
    )
    .replace('/', '_')
  ...
  try {
    const downloadRsp = await download(imgUrl, imgFilePath)

    if (downloadRsp.code === 0) {
      logger.info(
        `下载成功，耗时 ${(new Date().getTime() - downloadStartTime) / 1000} 秒`
      )
      downloadSuccessCount++
      downloadSuccessList.push(imgUrl)
    } else {
      logger.warn(
        `下载失败，错误码 ${downloadRsp.code}，错误信息 ${downloadRsp.msg}`
      )
      downloadFailedCount++
